...
//         let image = new Image()
//         let string = "" + k
//         while ((string).split("").length < 5) {
//             string = "0" + string
//         }
//         // image.src =`data/${letters[t]}/hsf_0/hsf_0_${string}.png`
//         frame.push(image)
//     }
//     images.push(frame)
// }
...
class Perceptron {
    constructor(inputs, kind) {
        this.bias = ((Math.random() - .5) * 2) * .5
    ...
    relu(vlain) {
        let value = Math.min(Math.max(vlain, net.reluslime), 1)
        return value
    }
    sig(vlain) {
...
class Network {
    constructor(inputs, layerSetupArray) {
        this.reluslime = 0.000001
        this.momentum = .5
        this.learningRate = .025
        ...
        for (let t = 0; t < layerSetupArray.length; t++) {
            ...
            for (let k = 0; k < layerSetupArray[t]; k++) {
                let cept
                if (t == 0) {
                    cept = new Perceptron(this.inputs, 0)
                } else {
                    if (t == layerSetupArray.length - 1) {
                        cept = new Perceptron(this.structure[t - 1], 0) //reluset?
                    } else[

                        cept = new Perceptron(this.structure[t - 1], 0)
                    ]
                }
                scaffold.push(cept)
            }
    ...
    calculateDeltasRELU(goals) {
        for (let t = this.structure.length - 1; t >= 0; t--) {
            ...
            for (let k = 0; k < layer.length; k++) {
                ...
                // perceptron.setDelta(error * (output > 0 ? 1 : 0)) 

                perceptron.setDelta(error * (output > net.reluslime ? Math.min(output, 1) : net.reluslime))
            }
        }
    }

    calculateDeltasDynamic(goals) {
        ...
        for (let t = this.structure.length - 1; t >= 0; t--) {
            ...
            for (let k = 0; k < layer.length; k++) {
                ...
                perceptron.setError(error)
                if (perceptron.kind == 0) {
                    perceptron.setDelta(error * output * (1 - output))
                } else {
                    perceptron.setDelta(error * (output > net.reluslime ? Math.min(output, 1) : net.reluslime) * (1 - (output > net.reluslime ? Math.min(output, 1) : net.reluslime)))  //?
                }
            }
        }
...
let net = new Network(inputs, [256, 128, 64, 10])
...
function exportJSON(originalData) {
    ...
    a.setAttribute("download", `128DiscRelNet.js`);
...
function main() {
    ...
    if (fling == 1) {
        ...
        for (let w = 0; w < net.structure[1].length; w++) {
            ...
            for (let k = 0; k < outpix.data.length; k += 4) {
                ...
                bloat2.push(box[Math.floor(k / 4)][1])
            }
            ...
            for (let k = 0; k < outpix.data.length; k += 4) {
                ...
                outpix.data[k] = (box[Math.floor(k / 4)][0] / max1) * 255
                outpix.data[k + 1] = (box[Math.floor(k / 4)][1] / max1) * 255
                outpix.data[k + 2] = box[Math.floor(k / 4)][2]
                outpix.data[k + 3] = box[Math.floor(k / 4)][3]
            }
        ...
        for (let q = 0; q < net.structure[2].length; q++) {
            ...
            for (let k = 0; k < outpix.data.length; k += 4) {
                ...
                outpix.data[k] = parseFloat(box[Math.floor(k / 4)][0] / max1) * 255
                outpix.data[k + 1] = parseFloat(box[Math.floor(k / 4)][1] / max1) * 255
                outpix.data[k + 2] = parseFloat(box[Math.floor(k / 4)][2])
                outpix.data[k + 3] = parseFloat(box[Math.floor(k / 4)][3])
            }
        ...
        //     for (let r = 0; r < net.structure[1].length; r++) {
        //         mat3[t].push([])
        //         for (let q = 0; q < net.structure[0].length; q++) {
        //             mat3[t][r].push([])
        //             for (let k = 0; k < pix1.data.length; k += 4) {
        //                 mat3[t][r].push(0)
        //             }
        ...
        for (let t = 0; t < net.structure[0].length; t++) {
            ...
            for (let k = 0; k < pix1.data.length; k += 4) {
                pix1.data[k] = net.structure[0][t].weights[(Math.floor(k / 4))].valueOf() * 255
                pix1.data[k + 1] = -net.structure[0][t].weights[(Math.floor(k / 4))].valueOf() * 255
                pix1.data[k + 2] = 64
                pix1.data[k + 3] = 255
            }
        ...
        for (let t = 0; t < net.structure[1].length; t++) {
            ...
            for (let r = 0; r < net.structure[0].length; r++) {
                ...
                for (let k = 0; k < pix1.data.length; k += 4) {
                    let val = 0
                    let m = net.structure[0][r].weights[(Math.floor(k / 4))].valueOf()
                    let n = net.structure[1][t].weights[r].valueOf()
                    val += m * n
                    vzal.push(val)
                }
                vals.push(vzal)
            ...
            for (let k = 0; k < pix1.data.length; k += 4) {
                ...
                pix1.data[k] = vals[t][(Math.floor(k / 4))] * 255
                pix1.data[k + 1] = -vals[t][(Math.floor(k / 4))] * 255
                pix1.data[k + 2] = 64
                // pix1.data[k] = vals[t][(Math.floor(k/4))]*255
                // pix1.data[k+1] = net.structure[1][t].weights[(Math.floor(k/4))].valueOf()*255
                // pix1.data[k+2] = net.structure[1][t].weights[(Math.floor(k/4))].valueOf()*255
                pix1.data[k + 3] = 255
            }

            canvas_context.putImageData(pix1, 256 + 270 + ((t % 8) * 34) + 32, 10 + Math.floor(t / 8) * 34)
        ...
        for (let t = 0; t < net.structure[2].length; t++) {
            ...
            for (let r = 0; r < net.structure[1].length; r++) {
                ...
                for (let k = 0; k < pix1.data.length; k += 4) {
                    let val = 0
                    // let m =net.structure[1][r].weights[(Math.floor(k/32))].valueOf()
                    let n = net.structure[2][t].weights[r].valueOf()
                    val += n * outvals[r][Math.floor(k / 4)]
                    vzal.push(val)
                }
                vals.push(vzal)
            ...
            canvas_context.putImageData(pix1, 256 + 520 + ((t % 8) * 34) + 94, 10 + Math.floor(t / 8) * 34)
        ...
        for (let t = 0; t < net.structure[3].length; t++) {
            ...
            for (let r = 0; r < net.structure[2].length; r++) {
                ...
                for (let k = 0; k < pix1.data.length; k += 4) {
                    let val = 0
                    // let m =net.structure[1][r].weights[(Math.floor(k/32))].valueOf()
                    let n = net.structure[3][t].weights[r].valueOf()
                    val += n * zigvals[t][Math.floor(k / 4)]
                    vzal.push(val)
                }
                vals.push(vzal)
            ...
            canvas_context.putImageData(pix1, 256 + 542 + 270 + ((t % 10) * 34) + 96, 10 + Math.floor(t / 10) * 34)

            canvas_context.fillStyle = "yellow"
            canvas_context.font = "15px comic sans ms"
            canvas_context.fillText(t, 542 + 256 + 270 + ((t % 10) * 34) + 96 + 10, 10 + (Math.floor(t / 10) * 34) + 50)
        ...
        for (let t = 0; t < net.structure[0].length; t++) {
            ...
            canvas_context.putImageData(pix1, ((t % 8) * 34) + 5, 10 + Math.floor(t / 8) * 34)
        ...
        for (let t = 0; t < net.structure[1].length; t++) {
            ...
            canvas_context.putImageData(pix1, 270 + ((t % 8) * 34) + 32, 10 + Math.floor(t / 8) * 34)
        ...
        for (let t = 0; t < net.structure[2].length; t++) {
            ...
            canvas_context.putImageData(pix1, 520 + ((t % 8) * 34) + 94, 10 + Math.floor(t / 8) * 34)
        ...
        for (let t = 0; t < net.structure[3].length; t++) {
            ...
            for (let r = 0; r < net.structure[2].length; r++) {
                ...
                for (let k = 0; k < pix1.data.length; k += 4) {
                    ...
                    // let m =net.structure[1][r].weights[(Math.floor(k/32))].valueOf()
                    let n = net.structure[3][t].weights[r].valueOf()
                    val += n * zigvals[r][Math.floor(k / 4)]
                    vzal.push(val)
                }
                vals.push(vzal)
            ...
            for (let k = 0; k < pix1.data.length; k += 4) {
                ...
                pix1.data[k] = (vals[t][(Math.floor(k / 4))] * 255) * 2
                pix1.data[k + 1] = -(vals[t][(Math.floor(k / 4))] * 255) * 2
                pix1.data[k + 2] = 64
                // pix1.data[k] = vals[t][(Math.floor(k/4))]*255
                // pix1.data[k+1] = net.structure[1][t].weights[(Math.floor(k/4))].valueOf()*255
                // pix1.data[k+2] = net.structure[1][t].weights[(Math.floor(k/4))].valueOf()*255
            ...
            canvas_context.putImageData(pix1, 542 + 270 + ((t % 8) * 34) + 96, 10 + Math.floor(t / 8) * 34)

            // canvas_context.fillStyle = "yellow"
            // canvas_context.font = "15px comic sans ms"
            // canvas_context.fillText(letters[t], 542+256+270+((t%10)*34) + 96+10,10+ (Math.floor(t/10)*34)+50)
        ...
        for (let t = 0; t < net.structure[4].length; t++) {
            let vals = []
            for (let r = 0; r < net.structure[3].length; r++) {
                let vzal = []
                for (let k = 0; k < pix1.data.length; k += 4) {
                    let val = 0
                    // let m =net.structure[1][r].weights[(Math.floor(k/32))].valueOf()
                    let n = net.structure[4][t].weights[r].valueOf()
                    val += n * subvasl[r][Math.floor(k / 4)]
                    vzal.push(val)
                }
                vals.push(vzal)
            ...
            canvas_context.putImageData(pix1, 1200 + ((t % 6) * 60), Math.floor(t / 6) * 40)
            canvas_context.fillStyle = "yellow"
            canvas_context.font = "10px comic sans ms"
            if (t > 36) {
                canvas_context.fillText(letters[t].split('')[0], 1200 + ((t % 6) * 60) + 35, (Math.floor(t / 6) * 40) + 20)
    ...
    if (start >= 1) {
        ...
        //     for (let t = 0; t < statpixboxes[count].data.length; t += 1) {F
        ...
        canvas_context.fillStyle = "white"
        canvas_context.fillRect(0, 0, 32, 32)
        canvas_context.drawImage(images[count % 62][Math.floor(count / 62)], 0, 0, 128, 128, (Math.random() - .5) * 5, (Math.random() - .5) * 5, 32, 32)
        statpixboxes[0] = canvas_context.getImageData(0, 0, 32, 32)
        inputs = []
        for (let t = 0; t < statpixboxes[0].data.length; t += 1) {
            if (t % 4 == 1) {
                if (Math.random() < .05) {

                    inputs.push(new Data(Math.max(Math.min((statpixboxes[0].data[t] / 255) + (Math.random() - .5), 1), 0)))
                } else {

                    inputs.push(new Data(statpixboxes[0].data[t] / 255))
                }
        ...
        if (start == 1) {
            ...
            let mobe = []
            for (let t = 0; t < 62; t++) {
                mobe.push(net.outputs[t])
                if (Math.abs(net.outputs[t]) > ds) {
                    i = t
                    ds = Math.abs(net.outputs[t])
                }
            }
            for (let t = 0; t < 62; t++) {
                if (t != count % 62) {
                    exputs.push(new Data(0))
                    diff += Math.abs(net.outputs[t])
                    rovediff += Math.abs(net.outputs[t])
                } else {
                    exputs.push(new Data(1))
                    diff += Math.abs(1 - net.outputs[t])
                    rovediff += Math.abs(1 - net.outputs[t])
                }
            }
            if (Math.random() < .01) {
        ...
        // canvas_context.drawImage(images[count%62][Math.floor(count/62)], 0, 0, 128,128,(count % 13) * 32, Math.floor(count / 13) * 32,32,32) 
        ...
        for (let t = 0; t < net.structure[0].length; t++) {
            ...
            for (let k = 0; k < pix1.data.length; k += 4) {
                pix1.data[k] = net.structure[0][t].weights[(Math.floor(k / 4))]
                pix1.data[k + 1] = net.structure[0][t].weights[(Math.floor(k / 4))]
                pix1.data[k + 2] = net.structure[0][t].weights[(Math.floor(k / 4))]
                pix1.data[k + 3] = 255
            }

            canvas_context.putImageData(pix1, (t % 8) * 34, Math.floor(t / 8) * 34)
